...
class Category(models.Model):
    ...
    logo = models.ImageField(upload_to="categories/", verbose_name="Логотип", null=True, blank=True)
...
class Product(models.Model):
    ...
    logo = models.ImageField(upload_to="products/logos/", verbose_name="Логотип", null=True, blank=True)
    price = models.DecimalField(max_digits=10, decimal_places=2, verbose_name="Цена", null=True, blank=True)
    stock = models.PositiveIntegerField(default=0, verbose_name="Количество на складе")
    rating = models.PositiveSmallIntegerField(choices=RATING_CHOICES, default=5, verbose_name="Оценка (1–5)")
    is_favorites = models.BooleanField(default=False, verbose_name="В избранном")
...
class CheckoutOrder(models.Model):
    ...
    email = models.EmailField("Email")
    phone = models.CharField("Телефон", max_length=20)
    delivery_type = models.CharField("Способ доставки", max_length=20, choices=DELIVERY_CHOICES)
    country = models.CharField("Страна", max_length=100)
    city = models.CharField("Город", max_length=100)
    address = models.CharField("Адрес", max_length=255)
    postcode = models.CharField("Индекс", max_length=20, blank=True)
    ...
    delivery_eta_hours = models.PositiveIntegerField("Минимальный срок, ч", default=24)
    preferred_time = models.TimeField("Желаемое время доставки", null=True, blank=True)
    delivery_datetime = models.DateTimeField("Назначено на", null=True, blank=True)
    delivery_note = models.CharField("Примечание по доставке", max_length=255, blank=True)
    created_at = models.DateTimeField("Создан", auto_now_add=True)

    def __str__(self):
        return f"Заказ #{self.id} — {self.first_name}".strip()
