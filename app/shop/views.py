...
# from app.shop.serializers import ProductSerializer, ReviewsSerializer, CheckoutOrderSerializer, CheckoutCreateSerializer, ContactSerializers
...
class CheckoutView(APIView):
    ...
    authentication_classes = []      # ✅ Отключаем CSRF
    ...
    def get(self, request, *args, **kwargs):
        ...
        delivery_type = request.query_params.get("delivery_type", "standard")
        preferred_time_str = request.query_params.get("preferred_time")
        subtotal = sum(Decimal(str(it["price"])) * int(it["quantity"]) for it in cart.values())
        shipping_cost = Decimal("700") if delivery_type == "express" else Decimal("0")
        total = subtotal + shipping_cost
        ...
        delivery_dt = _compute_delivery_datetime(min_hours, preferred_time)
        delivery_note = (
            f"Товар будет доставлен через 24 часа. "
            f"Доставка назначена на {timezone.localtime(delivery_dt).strftime('%d.%m.%Y %H:%M')}."
        )
    ...
    def post(self, request, *args, **kwargs):
        cart = request.session.get("cart", {})
        serializer = CheckoutCreateSerializer(data=request.data, context={"cart": cart, "request": request})
        serializer.is_valid(raise_exception=True)
        order = serializer.save()

        # --- Telegram уведомление ---
        order_data = {
            "product_name": ", ".join([item.product.name for item in order.items.all()]),
            "quantity": sum([item.quantity for item in order.items.all()]),
            "user_name": order.first_name,
            "user_phone": order.phone,
            "user_address": f"{order.country}, {order.city}, {order.address}"
        }
